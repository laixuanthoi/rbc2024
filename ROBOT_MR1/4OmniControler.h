...
// int motorLock = 2;
// int motorLockRepeat = 200000;//20000
...
//		Mor_FrontRight_thuan;
// 	}else{
//		Mor_FrontRight = motorMinFix(-speed_FR);
//		Mor_FrontRight_nghich;
// 	}
...
{
	if (!speed_0h)
		Motor_Wheel_0h = motorLock;
	else if (speed_0h > 0)
	{
		Motor_Wheel_0h = speed_0h;
		Motor_Wheel_0h_Next;
	}
	else
	{
	...
	if (!speed_4h)
		Motor_Wheel_4h = motorLock;
	else if (speed_4h > 0)
	{
		Motor_Wheel_4h = speed_4h;
		Motor_Wheel_4h_Next;
	}
	else
	{
	...
	if (!speed_8h)
		Motor_Wheel_8h = motorLock;
	else if (speed_8h > 0)
	{
		Motor_Wheel_8h = speed_8h;
		Motor_Wheel_8h_Next;
	}
	else
	{
...
void setAllMotor(int speed)
{
	Motor_Wheel_0h = speed;
	Motor_Wheel_4h = speed;
	Motor_Wheel_8h = speed;
}
//------------------------------------------------------------------------------
void robotLock()
{
...
int robotIsStop()
{
	return (Motor_Wheel_0h < 3 && Motor_Wheel_4h < 3 && Motor_Wheel_8h < 3);
}
//------------------------------------------------------------------------------
int robotIsRun()
{
...
float fixMotorValue(int T)
{
	if (T == 0)
		...
		return (T * 0.1 - 0.1) / _robotCurrentSpeed; // tuy thuoc robot ma co the he so toi thieu la 0.5, 1, 2...
	else
		return (T * 0.1 + 0.1) / _robotCurrentSpeed;
}
//------------------------------------------------------------------------------
float absF(float num)
{
...
void calculateMotor(float rotate)
{
	...
	if (_robotCurrentSpeed > _robotMaxSpeed)
		_robotCurrentSpeed = _robotMaxSpeed;
	//------------------------------------------------
	_robotChange = 1;
}
//------------------------------------------------------------------------------
...
void robotRotate(int rotateAngle, float rotate, int rotatePoint)
{
	if (_robotRotateAngle != rotateAngle || _robotRotate != rotate || _robotRotatePoint != rotatePoint)
	{
		if (_robotAngle == 30000)
		{
			...
			{ // Quay theo goc no do
				_robotAngleCounterFix = 1;
				if ((rotate > 0 && _robotIMUAngle > rotateAngle) || (rotate < 0 && _robotIMUAngle < rotateAngle))
					rotate = -rotate;
			}
		}
...
void robotAngleAnalytics(void)
{
	if (_robotAngleCounterFix > 1)
	{
		...
		else
		{
			...
			if (_robotIMUAngle > _robotRotateAngle)
				_robotRotate = -1;
			else
				_robotRotate = 1;
...
{
	// Khi chay duong cong hoac khong chay hoac quay tu do thi thoat
	if (_robotCurve || !_robotCurrentSpeed || _robotRotateAngle >= 30000)
		return;
	// Khi co lenh xoay
	if (_robotRotate)
	{
		...
		{ // Vua chay thang vua xoay
				...
				(_robotRotate < 0 && _robotIMUAngle < _robotRotateAngle - _robotCurrentSpeed * _robotRotate + i100))
			{
				...
				calculateMotor(_robotRotate); //---- Neu chua dat den goc can quay ----------
		}
...
void ac_Stop(void)
{
	...
	//-------------------------------------
	if (++_robotCounter < motorLockRepeat)
...
void robotAnalytics(void) //-------- Dat trong Interup Timer ---------
{
	...
	if (_robotRunSpeed > 0) // Robot dang duoc ra lenh chay
	{
		if (_robotCurrentSpeed != _robotRunSpeed)
		{
			...
			{
				if (_robotCurrentSpeed < _robotMaxSpeed)
					ac_SpeedACC(giaToc(_robotCurrentSpeed), 1); //--- Phuong trinh Gia toc --------
			}
			else
				ac_SpeedACC(_robotDelayGiamToc, -1); //--- Phuong trinh Giam toc --------
		}
	}
	else //-------- Robot duoc lenh Stop -----------
		if (_robotCurrentSpeed > 0)
			ac_SpeedACC(_robotAccStop, -1); //-------- Giam toc do -----------
		else
		{
			ac_Stop();
			return;
	...
	//======= Thiet lap thong so dong co ==========
	if (_robotChange)
		setMotor(roundF(_robotCurrentSpeed * _robotRun0h), roundF(_robotCurrentSpeed * _robotRun4h), roundF(_robotCurrentSpeed * _robotRun8h));
}
