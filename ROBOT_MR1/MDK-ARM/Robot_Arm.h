...
void giu_Tay_X(void){
	...
	else { Motor_Arm_X_Speed = armX.minMotorSpeed;armX.isRunning = 0;}
}
...
void resetArmYToReady()
{
    ...
    for (i = 0; i < 50; i++) while (getEncoderArmY() < 1000) {vTaskDelay(1);}
    Motor_Arm_Y_Speed = armY.minMotorSpeed;
...
void resetArmGripperToReady(){
    ...
    for (i = 0; i < 50; i++) while(!isEqualBienTroGripper(gripper.tagetBienTro,temp)){
...
void gripperGetBallByXY(int x,int y){
    ...
    for (i = 0; i < 50; i++) while (armX.isRunning == 1 || base.isRunning == 1)
    {
        vTaskDelay(1);
    }
    temp = getBienTroGripper();
...
void runToSilo0Red(){
    ...
    for(i=0;i<50;i++) while (lazerTruocValue > 30){
    ...
    for(i=0;i<50;i++) while (lazerTruocValue > 5){vTaskDelay(1);}
    for(i=0;i<50;i++) while (abs(lazerTraiValue - 170) > 1)
    {
        if(lazerTraiValue > 170) robotRunAngle(-950,10,0,-0.3);
        else robotRunAngle(950,10,0,0.3);
    }
    robotStop(0);
